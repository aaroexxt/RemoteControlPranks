<?php
    ...
    if ($ID == "" || $IP == "" || $modTag == "" || $modValue == "") {
        echo "IP_ID_MODTAG_MODVALUE_BLANK";
    } else {
        //replace all strs encoded before
        $modValue = str_replace("&op>","(",$modValue);
        $modValue = str_replace("&cp>",")",$modValue);
        $modValue = str_replace("&ob>","[",$modValue);
        $modValue = str_replace("&cb>","]",$modValue);
        $modValue = str_replace("&oc>","{",$modValue);
        $modValue = str_replace("&cc","}",$modValue);
        //echo "SUCCESS";
        $root = realpath($_SERVER["DOCUMENT_ROOT"]);
        if (file_exists($root.'/remote/___remote.txt')) 
        {
            ...
            if ($deviceNum == null) {
                warn("UpdateDeviceParams: No device found");
                echo "ERROR_NO_DEVICE_FOUND";
            } else {
                if (updateDeviceValue($devices, $untrim_file, $deviceNum, $modTag, " ".trim(str_replace("'","",$modValue))) !== null) {
                    echo "success";
                } else {
                    echo "fail";
                }
            ...
            warn("No tracking file from modifyDevice.php");
        }
